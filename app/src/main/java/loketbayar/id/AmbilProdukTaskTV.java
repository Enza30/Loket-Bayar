...
class AmbilProdukTaskTV extends AsyncTask<String, Void, String> {
   private Context ctx;
    private String results;
    AmbilProdukTaskTV(Context ctx) {
        this.ctx = ctx;
    }
       ...
       // Toast.makeText(ctx, result, Toast.LENGTH_SHORT).show();
        try {
           if(result.contains("dataproduk")){
               ...
               Tvkabel.kode_produk_biller=kode_produk_biller.get(0);
               Tvkabel.kode_produk_biller=kode_produk_biller.get(0);
               ...
               if(fee_ca.isEmpty()){
                   ...
                   Tvkabel.fee_ca=fee_ca.get(0);
                   Tvkabel.fee_sub_ca=fee_sub_ca.get(0);
                   Tvkabel.fee_sub_ca_1=fee_sub_ca_1.get(0);
                   Tvkabel.fee_sub_ca_2=fee_sub_ca_2.get(0);
                   Tvkabel.fee_sub_ca_3=fee_sub_ca_3.get(0);
               }
               Tvkabel.dataproduk=results;
              runThread();
            }else{
               Dialog dialog=new Dialog(ctx);
               dialog.setContentView(R.layout.dialogok);
